> **MIGRATION NOTE (v5.0.8 upgrade):** This tree is now based on the official **QGroundControl v5.0.8 stable** release.
> The scand80gui customizations (Payload Drop, Navigation Lights, SBUS-noise filter, Windows bootstrap launcher) were
> replayed onto v5.0.8. Two structural adaptations vs. the previous master-based tree:
> (1) FlyView QML now lives in `src/FlightDisplay/` (upstream renamed `src/FlyView/` → `src/FlightDisplay/`);
> (2) toolchain pins changed to v5.0.8's: **Qt 6.8.3**, **NDK r26b**, **GStreamer 1.22.12** (see `.github/build-config.json`).
> Some references below to "master", Qt 6.10.3, or `src/FlyView/` reflect the prior base and are retained for history.
